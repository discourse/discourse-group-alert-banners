export default function migrate(settings, helpers) {
  const oldNavLinksSetting = settings.get("nav_links") || "[]";
  const newNavLinksSetting = JSON.parse(oldNavLinksSetting).map((link) => {
    const newLink = {
      group_name: link.group_name,
      group_exclude: link.group_exclude || "",
      content: link.content,
      button_url: link.button_url,
      button_text: link.button_text,
      button_icon: convertIconName(link.button_icon),
    };

    if (
      newLink.button_url &&
      (!helpers.isValidUrl(newLink.button_url) ||
        newLink.button_url.length > 2048)
    ) {
      newLink.button_url = "#";
    }

    ["group_name", "group_exclude", "content", "button_text"].forEach((key) => {
      if (newLink[key] && newLink[key].length > 1000) {
        newLink[key] = newLink[key].substring(0, 1000);
      }
    });

    return newLink;
  });
  settings.set("nav_links", newNavLinksSetting);
